  ...
  namespace 'import' do
    ...
    task 'all' => :environment do
      ...
      import_list.each do |item|
        ...
        begin
          Rake::Task["catherine:import:#{item}"].invoke
    ...
    ["Country", "Region", "ExchangeRate", "Supplier", "Store", "Brand", "Category", "Terminal"].each do |cls_name|
      ...
      task name => :environment do
        ...
        path = ENV['path']
        cls_name.constantize.import_data(Catherine::Engine, path, false, verbose)
      end
    end
    ...
    namespace 'dummy' do
      ...
      ["Country", "Region", "ExchangeRate", "Supplier", "Store", "Brand", "Category", "Terminal"].each do |cls_name|
        ...
        task name => :environment do
          verbose = true
          verbose = false if ["false", "f","0","no","n"].include?(ENV["verbose"].to_s.downcase.strip)
          cls_name.constantize.import_data(Catherine::Engine, nil, true, verbose)                
        end
      end
